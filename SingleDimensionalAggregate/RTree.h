...
public:
	...
	double GenerateMaxAggregate(Node* a_node);
...
double RTREE_QUAL::AggregateApprox(Node* a_node, Rect* a_rect, double Trel, bool &break_indicator, double total_inside, double total_upperbound, bool first_time) {
	...
	if (a_node->IsInternalNode()) {
		...
		vector<Branch> intersectedRects;
		int count_overlap = 0;
...
RTREE_TEMPLATE
double RTREE_QUAL::GenerateMaxAggregate(Node* a_node) {
	ASSERT(a_node);
	ASSERT(a_node->m_level >= 0);
	ASSERT(a_rect);
	...
	vector<double> aggmaxs;

	// if internal node
	if (a_node->IsInternalNode()) {
		// for every entry inside the node
		for (int index = 0; index < a_node->m_count; ++index) {
			temp = GenerateMaxAggregate(a_node->m_branch[index].m_child);
		...
		tempmax = a_node->m_branch[0].m_data;
		for (int index = 0; index < a_node->m_count; ++index) {
			if (a_node->m_branch[index].m_data > tempmax) {
				tempmax = a_node->m_branch[index].m_data;
			}
		}
		a_node->AggregateMax = tempmax;
	}
