...
public:
	...
	void MyCplexSolverForMaxLoss(vector<double> &key_v, vector<double> &position_v, double &a, double &b, double &loss) {
		IloEnv env;
		IloModel model(env);
		...
		vars.add(IloNumVar(env, -INFINITY, INFINITY, ILOFLOAT)); // the weight, i.e., a
	...
	void MyCplexSolverForMaxLossOptimized(vector<double> &key_v, vector<double> &position_v, int lower_index, int higher_index, double &a, double &b, double &loss) {
		IloEnv env;
		IloModel model(env);
		IloCplex cplex(model);
		IloObjective obj(env);
	...
	// using arma format
	void MyCplexSolverForMaxLoss2(const arma::mat& current_trainingset, const arma::rowvec&current_response, double &a, double &b, double &loss) {
		IloEnv env;
		IloModel model(env);
		IloCplex cplex(model);
		IloObjective obj(env);
	...
	// this method change the percentage threshold to absolute threshold (in max loss sampled points)
	void ApproximateMaxLossLinearRegression2(int origin_index, int shift_index, vector<double> &key_v, vector<double> &position_v, const arma::mat& current_trainingset, const arma::rowvec&current_response, double sampling_control, double error_threshold, double sampling_points, double &slope, double &intercept, double &maxloss) {
	...
	// this method change the percentage threshold to absolute threshold (in max loss sampled points)
	void ApproximateMaxLossLinearRegressionMultipass(int origin_index, int shift_index, vector<double> &key_v, vector<double> &position_v, const arma::mat& current_trainingset, const arma::rowvec&current_response, double sampling_control, double error_threshold, double sampling_points, double &slope, double &intercept, double &maxloss, int pass=1) {

		auto t0 = chrono::steady_clock::now();

		double a, b, maxerr;
		for (int pass_i = 0; pass_i < pass; pass_i++) {

			// 1. first train an linear regression as the axes, if not the first time, use the previous model
			if (pass_i == 0) {
				LinearRegression lr(current_trainingset, current_response);
				arma::vec paras = lr.Parameters();
				a = paras[1]; // a
			...
			// 4. train an max loss linear regression based on these points
			arma::mat selected_points_key = current_trainingset.cols(selected_index_arma);
			arma::rowvec selected_points_pos = current_response.cols(selected_index_arma);

			// 5. adjust the bias term to balance the error
			MyCplexSolverForMaxLoss2(selected_points_key, selected_points_pos, a, b, maxerr);
			...
			double adjusted_b = b + (max_pos_err - abs(max_neg_err)) / 2;
			double max_final_err = (max_pos_err - max_neg_err) / 2;
			//cout << "max error for approx max loss:" << maxerr << endl;

			slope = a;
			intercept = adjusted_b;
			maxloss = max_final_err;

			//cout << "Pass: " << pass_i << "    Sampled Point LP Maxloss: " << maxerr << "    Final Max Error: " << max_final_err << endl;
		}
	...
	// do not need to dump parameter for the bottom layer anymore
	void TrainBottomLayerWithFastDetectForMaxLoss(const arma::mat& dataset, const arma::rowvec& labels, int step = 1000, double sampling_control=2, int sampling_points = 5) {
		// Todo
		int layer = this->level - 1;
		...
		while (origin_index < TOTAL_SIZE) {
			...
			//cout << origin_index << " " << shift_index << endl;
			...
			//ApproximateMaxLossLinearRegression2(origin_index, shift_index, key_v, position_v, current_trainingset, current_response, sampling_control, threshold, sampling_points, a, b, current_absolute_accuracy);
			
			//using linear programming
			//MyCplexSolverForMaxLoss(key_v, position_v, a, b, current_absolute_accuracy);
			MyCplexSolverForMaxLoss2(current_trainingset, current_response, a, b, current_absolute_accuracy);
	...
	// the greedy but optimal segmentation
	void TrainBottomLayerWithSegmentOnTrainMaxLoss(const arma::mat& dataset, const arma::rowvec& labels) {
		...
		while (origin_index < TOTAL_SIZE) {
			...
			while (current_absolute_accuracy < threshold && shift_index < TOTAL_SIZE-1) {
				...
				current_trainingset = dataset.cols(origin_index, shift_index);
				current_response = labels.cols(origin_index, shift_index);
	...
	// using linear programming, the original version is kind of slow, try to avoid data passing and extraction
	void TrainBottomLayerWithSegmentOnTrainMaxLossOptimized(const arma::mat& dataset, const arma::rowvec& labels) {
		...
		while (origin_index < TOTAL_SIZE) {
			...
			int Co = shift_index - origin_index;
			MyCplexSolverForMaxLossOptimized(key_v, position_v, origin_index, shift_index, a, b, current_absolute_accuracy);
	...
	// using the approximate max loss algorithm
	void TrainBottomLayerWithSegmentOnTrainApproximateMaxLossOptimized(const arma::mat& dataset, const arma::rowvec& labels, int sampling_points=20) {
		int layer = this->level - 1;
		index_range.clear();
		...
		while (origin_index < TOTAL_SIZE) {
			...
			current_trainingset = dataset.cols(origin_index, shift_index); // included [origin_index, shift_index]
			current_response = labels.cols(origin_index, shift_index);

			ApproximateMaxLossLinearRegressionMultipass(origin_index, shift_index, key_v, position_v, current_trainingset, current_response, 1, threshold, sampling_points, a, b, current_absolute_accuracy);

			//ApproximateMaxLossLinearRegression2(origin_index, shift_index, key_v, position_v, current_trainingset, current_response, 1, threshold, sampling_points, a, b, current_absolute_accuracy);
	...
	void SelfChecking(vector<double> &keys, vector<double > &positions) {
		...
		for (int i = 0; i < keys.size(); i++) {
			...
			iter = this->bottom_layer_index.upper_bound(keys[i]); // find the node greater or equal than the search key
			iter--;
			model_index = iter->second;
			//cout << queryset[i] << " " << model_index << endl;
			...
			result = a * keys[i] + b;
			//results.push_back(result);
			if (abs(result - positions[i]) > threshold) {
				cout << "error in key[" << i << "]: " << keys[i] << "  predicted: " << result << "  actual: " << positions[i]  << "  difference: " << abs(result-positions[i]) << " model index: " << model_index << endl;
			}
		}
	}
