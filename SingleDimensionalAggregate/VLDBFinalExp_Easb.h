...
// Experiment 1: Time - Eabs (no refinement)
...
void VLDB_Final_Experiment_0_COUNT() {
	...
	run_result.open("C:/Users/Cloud/iCloudDrive/LearnedAggregate/VLDB_Final_Experiments/RunResults/Exp0_COUNT.csv", std::ios::app);

	// FITingTree
	QSS.clear();
	for (int i = 0; i < Eabs_collection.size(); i++) {
		Eabs = Eabs_collection[i];
		QS = TestFITingTree(keys, values, query_low, query_up, 0.01, Eabs, false);
		QSS.push_back(QS);
	}
	//store it in file
	...
	//for (int i = 0; i < Eabs_collection.size(); i++) {
	//	Eabs = Eabs_collection[i];
	//	QS = TestRMI(keys, values, query_low, query_up, 0.01, Eabs, false); // the detailed settings are inside this method, along with the dataset it used !
	//	QSS.push_back(QS);
	//}
	////store it in file
	//for (int i = 0; i < QSS.size(); i++) {
	...
	//for (int i = 0; i < Eabs_collection.size(); i++) {
	//	Eabs = Eabs_collection[i];
	//	QS = TestPolyfit(keys, values, query_low, query_up, 0.01, Eabs, 1, false); // probability= 0.9, Trel = 0.01, double Tabs = 100, int highest_term
	//	QSS.push_back(QS);
	//}
	////store it in file
	//for (int i = 0; i < QSS.size(); i++) {
	...
	//for (int i = 0; i < Eabs_collection.size(); i++) {
	//	Eabs = Eabs_collection[i];
	//	QS = TestPolyfit(keys, values, query_low, query_up, 0.01, Eabs, 2, false); // probability= 0.9, Trel = 0.01, double Tabs = 100, int highest_term
	//	QSS.push_back(QS);
	//}
	////store it in file
	//for (int i = 0; i < QSS.size(); i++) {
	...
	for (int i = 0; i < Eabs_collection.size(); i++) {
		Eabs = Eabs_collection[i];
		QS = TestPolyfit(keys, values, query_low, query_up, 0.01, Eabs, 3, false); // probability= 0.9, Trel = 0.01, double Tabs = 100, int highest_term
		QSS.push_back(QS);
	}
	//store it in file
	for (int i = 0; i < QSS.size(); i++) {
...
void VLDB_Final_Experiment_0_MAX() {
	...
	std::ofstream run_result;
	run_result.open("C:/Users/Cloud/iCloudDrive/LearnedAggregate/VLDB_Final_Experiments/RunResults/Exp0_MAX.csv", std::ios::app);
...
void VLDB_Final_Experiment_0_COUNT2D() {
	...
	std::ofstream run_result;
	run_result.open("C:/Users/Cloud/iCloudDrive/LearnedAggregate/VLDB_Final_Experiments/RunResults/Exp0_COUNT2D.csv", std::ios::app);

	QSS.clear();
	// Polyfit, use its own accumulation dataset, set in its own method
	for (int i = 0; i < Eabs_collection.size(); i++) {
		Eabs = Eabs_collection[i];
		QS = TestPolyfit_COUNT2D(keys1, keys2, query_low1, query_low2, query_up1, query_up2, 0.01, Eabs, false);
		QSS.push_back(QS);
	}
	//store it in file
	for (int i = 0; i < QSS.size(); i++) {
